...
1. **运行时配置文件**  
   默认：`%APPDATA%\AskMyZotero\config.yaml`（例如：`C:\Users\用户名\AppData\Roaming\AskMyZotero`）。  
...
2. **索引与切块工作目录**（默认相对 `work_dir` + `index_name`，见 `config.yaml` / `config.dist.yaml`）  
   - **脚本启动**：一般在项目根目录下的 **`.askmyzotero\default\`**（内含 `faiss_index`、`zotero_splits_cache.pkl`、`manifest.json` 等）。  
   - **exe 启动**：一般在 **`AskMyZotero.exe` 同目录下** 的 `.askmyzotero\default\`（因 frozen 时相对路径相对 exe 目录）。  
...
![测试效果截图](https://github.com/user-attachments/assets/e8dee2ca-81a8-4c06-a397-1e83200d4112)

### 本地开发测试

1. 激活你的 Python 环境后，在项目根目录执行：python server.py
...
### 前端阶段性优化：配置持久化
- **`settings.html`** 写入后端配置项；页面均通过 **`http://127.0.0.1:8000/`** 访问
- 运行时配置默认 **`%APPDATA%\AskMyZotero\config.yaml`**，可用 **`ASKMYZOTERO_CONFIG`** 环境变量覆盖；**`GET /api/config`** 返回 **`api_key_set`**（不明文）；保存时 **API Key 留空 = 不覆盖**；**`localStorage` 不存 Key**，非敏感字段可缓存，以服务端为准。
...
- 新增 `indexer.py` 中 **`api_ask_once` 函数** 用于一次性获取回答
- 新增 **`schemas.py`** 脚本进行前后端的数据结构设定和检查（注：目前仅有问询和回答，其他根据前端渲染文献卡片的信息设置，但后端还并未同步得到这些信息的功能）
- 修改 `zotero_rag_ui.html` 中的 `runSearch` 函数，新增 **`renderResults` 函数** 用于展示回答

### 测试方法
1. 配置好 `config.yaml`
2. 运行 `server.py` 脚本
3. 进入网页输入在你的文件里面能搜到的问题

### 测试结果
![测试结果截图](https://github.com/user-attachments/assets/9803f85b-0455-4ef4-825f-17fe25c0a756)
